...
async function prepareBatch12() {
  console.log('Fetching 100 unprocessed candidates for Batch 12...');
  ...
  fs.writeFileSync(path.join(baseDir, 'batch12_candidates.json'), JSON.stringify(candidates, null, 2), 'utf8');

  // Chunk into 20 groups of 5
  for (let i = 0; i < 20; i++) {
    const chunk = candidates.slice(i * 5, (i + 1) * 5);
    const chunkFile = path.join(baseDir, `batch12_chunk_${i + 1}.json`);
    fs.writeFileSync(chunkFile, JSON.stringify(chunk, null, 2), 'utf8');
    console.log(`Batch 12 Chunk ${i + 1}: ${chunk.map(c => c.empresa).join(', ')}`);
  }
}

prepareBatch12().catch(console.error);
