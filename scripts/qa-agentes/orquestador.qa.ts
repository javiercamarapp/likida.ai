...
import {
  ...
  drenarSalientes, type MensajeSaliente,
...
const ESCENARIOS = new Map<AtaqueOperador, EscenarioOperador>();
...
function guardarEvidencia(
  esc: EscenarioOperador, rep: number,
  contenidos: Record<string, unknown>,
  pngs: Record<string, string> = {},
): string {
  const dir = dirEvidencia(FECHA, esc.ataque, rep);
...
async function correrRep(esc: EscenarioOperador, rep: number): Promise<ResultadoRep> {
  ...
  return { ataque: esc.ataque, rep, veredictos, costoUsd, rutaEvidencia, nota };
}
...
function escribirReporte(): string {
  ...
  for (const ataque of ATAQUES_FASE_1) {
    ...
    const lineas = [
      ...
      `- **Datos sembrados**: anticipo $${esc.anticipo.toFixed(2)}, viaje \`${esc.folioViaje}\`${esc.folioViaje2 ? ` + \`${esc.folioViaje2}\`` : ''}, tickets ${esc.tickets.map((t) => `$${t.monto.toFixed(2)} (${t.fecha}, ${t.folio})`).join(' · ') || '(ninguno)'}${esc.textos.length ? `, textos: ${esc.textos.map((t) => `«${t}»`).join(', ')}` : ''}`,
      '',
