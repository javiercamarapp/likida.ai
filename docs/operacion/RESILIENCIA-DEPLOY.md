...
1. Preflight de `VERCEL_TOKEN`, `VERCEL_ORG_ID` y `VERCEL_PROJECT_ID`.
2. Link + pull + build de Preview.
3. Deploy `--prebuilt` y captura de la URL exacta.
4. `npm run test:smoke` contra esa URL.
5. Solo en `workflow_dispatch`, y solo con `promote=true`, un job con
   environment `production` promueve exactamente la Preview testeada.

Configurar en GitHub el environment `production` con al menos un reviewer. En
pull requests desde forks el despliegue se omite porque GitHub no expone
secretos a código no confiable. En ejecuciones confiables sin credenciales el
preflight falla; no se simula un deploy exitoso.
