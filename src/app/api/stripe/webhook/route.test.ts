...
// desmarcar se importa dinámicamente dentro del route; `borrado` captura el
// delete sobre evento_stripe para poder afirmar que el marker se quitó.
...
describe('POST /api/stripe/webhook — la puerta del plan', () => {
  ...
  it('si aplicar falla: 500 (el reintento de Stripe podrá volver a aplicar)', async () => {
    ...
    expect(borrado).toHaveBeenCalledWith('evento_stripe', 'id', 'evt-1');
  });
});
...
// tiene que lanzar: 500 + marker de idempotencia borrado = el reintento entra.
describe('las ramas de fallo lógico LANZAN para entrar al camino de retry', () => {
  it('checkout sin tenant (ni client_reference_id ni metadata): 500 y marker borrado', async () => {
    const r = await POST(req({
      id: 'evt-c1', type: 'checkout.session.completed',
      data: { object: { subscription: 'sub-1' } },
    }));
    expect(r.status).toBe(500);
    expect(borrado).toHaveBeenCalledWith('evento_stripe', 'id', 'evt-c1');
  });

  it('checkout sin subscription: 500 y marker borrado (nuestros checkouts siempre son de suscripción)', async () => {
    const r = await POST(req({
      id: 'evt-c2', type: 'checkout.session.completed',
      data: { object: { client_reference_id: 't-1' } },
    }));
    expect(r.status).toBe(500);
    expect(borrado).toHaveBeenCalledWith('evento_stripe', 'id', 'evt-c2');
  });
  ...
  it('suscripción sin tenant (sin metadata y customer desconocido): 500, NO aplica, marker borrado', async () => {
    ...
    expect(r.status).toBe(500);
    expect(aplicarSuscripcion).not.toHaveBeenCalled();
    expect(borrado).toHaveBeenCalledWith('evento_stripe', 'id', 'evt-s1');
  });

  it('price sin plan que le corresponda: 500, NO aplica, marker borrado', async () => {
    ...
    expect(r.status).toBe(500);
    expect(aplicarSuscripcion).not.toHaveBeenCalled();
    expect(borrado).toHaveBeenCalledWith('evento_stripe', 'id', 'evt-s2');
  });

  it('invoice sin tenant (customer desconocido): 500, NO registra, marker borrado', async () => {
    tenantDeCustomer.mockResolvedValueOnce(null);
    const r = await POST(req({
    ...
    expect(borrado).toHaveBeenCalledWith('evento_stripe', 'id', 'evt-f1');
  });
