...
export async function ejecutarAnalista(opts: {
  ...
  try {
    ...
    if (!bloques) bloques = [{ tipo: 'texto', texto: AVISO_SIN_RESPALDO }];

    if (!cifrasRespaldadas(bloques, respaldo)) {
      // Fail-closed sin reintento: repreguntarle al modelo duplica el costo
      // y suele repetir la invención. Mejor honestidad barata.
      logger.warn('chat.guardia_bloqueo', { tenantId: opts.tenantId });
