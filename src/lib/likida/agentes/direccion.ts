...
async function incidenteSinParte(): Promise<Incidente | null> {
  const { data, error } = await acotada(supabaseAdmin()
    .from('incidencia')
...
async function correrEspecialistasIncidente(disparo: DisparoCorrida, hoy: string): Promise<ResultadoDireccionBandeja> {
  const inicio = new Date();
  const agente = 'especialistas_incidente';
  try {
    const inc = await incidenteSinParte();
    if (inc === null) {
      await anotarCorrida(agente, inicio, 'ok', disparo, { pieza: 'sin_incidente' });
      return {
...
export async function correrAgenteDireccionBandeja(
  ...
  switch (id) {
    ...
    case 'especialistas_incidente': return correrEspecialistasIncidente(disparo, hoy);
