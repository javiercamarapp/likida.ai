...
async function candidatoFicha(): Promise<DatosFicha | null> {
  const { data, error } = await acotada(supabaseAdmin()
    .from('prospecto')
  ...
  // que es la misma clave que arbitra el índice único de la 0235.
...
async function correrDossier(disparo: DisparoCorrida, hoy: string): Promise<ResultadoLeads> {
  const inicio = new Date();
  const agente = 'dossier';
  try {
    const d = await candidatoFicha();
    if (d === null) {
      await anotarCorrida(agente, inicio, 'ok', disparo, { pieza: 'sin_candidato' });
      return {
...
async function candidatoDemo(): Promise<DatosDemo | null> {
  const { data, error } = await acotada(supabaseAdmin()
    .from('prospecto')
...
async function correrDemoPrep(disparo: DisparoCorrida, hoy: string): Promise<ResultadoLeads> {
  const inicio = new Date();
  const agente = 'demo_prep';
  try {
    const d = await candidatoDemo();
    if (d === null) {
      await anotarCorrida(agente, inicio, 'ok', disparo, { pieza: 'sin_candidato' });
      return {
        resultado: 'corrio', piezas: 0, costoUsd: 0,
...
async function candidatoPropuesta(): Promise<DatosPropuesta | null> {
  const { data, error } = await acotada(supabaseAdmin()
    .from('prospecto')
...
async function correrPropuestas(disparo: DisparoCorrida, hoy: string): Promise<ResultadoLeads> {
  const inicio = new Date();
  const agente = 'propuestas';
  try {
    const d = await candidatoPropuesta();
    if (d === null) {
      await anotarCorrida(agente, inicio, 'ok', disparo, { pieza: 'sin_candidato' });
      return {
        resultado: 'corrio', piezas: 0, costoUsd: 0,
...
export async function correrAgenteLeads(
  ...
  switch (id) {
    ...
    case 'dossier': return correrDossier(disparo, hoy);
    case 'vigia': return correrVigia(disparo, hoy);
    case 'demo_prep': return correrDemoPrep(disparo, hoy);
    case 'propuestas': return correrPropuestas(disparo, hoy);
