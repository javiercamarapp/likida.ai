...
type Fila = { id: string; evento: { from: string; type: 'text' }; intentos: number; lease_token: string; lease_owner: string; lease_until: string };
...
beforeEach(() => {
  ...
  rpc.mockImplementation(async (nombre: string, args: Record<string, unknown>) => {
    if (nombre === 'claim_wa_evento_pendiente') {
    ...
    if (nombre === 'renew_wa_evento_pendiente') {
      const fila = vigente;
      const ok = fila?.lease_token === args.p_lease_token && fila?.lease_owner === args.p_lease_owner;
      return { data: ok ? true : false, error: null };
    }
    if (nombre === 'complete_wa_evento_pendiente') {
      const fila = vigente;
      const ok = fila?.lease_token === args.p_lease_token && fila?.lease_owner === args.p_lease_owner;
      if (ok) vigente = null;
