...
describe('getUltimasPosiciones — la posición medida, sin traerse `posicion`', () => {
  it('llama a la RPC de la 0266 con su tenant y NO pagina la tabla', async () => {
  ...
  it('la 0266 sin aplicar (otra forma) LANZA con la migración en el mensaje', async () => {
    respPosiciones = { data: { filas: [] }, error: null };
    await expect(getUltimasPosiciones('t-1')).rejects.toThrow(/0266 sin aplicar/);
  });
