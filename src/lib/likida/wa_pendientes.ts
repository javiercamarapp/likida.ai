...
/**
 * Persiste los mensajes de una invocación con el sistema apagado. NUNCA
 * lanza: corre dentro del after() con el 200 ya emitido — aquí ya no hay a
 * quién contestarle un error. Un insert fallido ES pérdida real y por eso
...
export async function guardarEventosPendientes(mensajes: InboundMessage[]): Promise<{ guardados: number; fallidos: number }> {
  ...
  for (const m of mensajes) {
    ...
    const id = m.waMessageId ?? `sin-wamid:${m.from}:${Date.now()}`;
    try {
