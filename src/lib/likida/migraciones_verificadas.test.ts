...
const EXENTAS: Record<string, string> = {
  ...
  '0185': 'runtime de agentes: la unicidad de `(tenant_id, effect_key)`, RLS deny-all y los grants service_role, junto con los RPCs de claim/fencing y reserva/liquidación, tienen contrato SQL estático cubierto por `src/lib/llm/migration_0185.test.ts`; la carrera real se ejecuta en Postgres dentro de la RPC.',
  '0186': 'leases/fencing de WhatsApp: la migración añade token, owner y expiración a inbox e idempotencia downstream, RPCs server-only con FOR UPDATE SKIP LOCKED, renovación y complete/fail condicionados al token. El contrato SQL y las carreras observables están en `supabase/tests/wa_leases_fencing.sql` y el contrato de adaptadores en `src/lib/likida/wa_pendientes_leases.test.ts`; la carrera de dos sesiones se ejecuta en Postgres local/CI.',
};
