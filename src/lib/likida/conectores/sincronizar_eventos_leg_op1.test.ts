...
// ═══════════════════════════════════════════════════════════════════════════
// AUDITORÍA 25, LEG-OP-1 (CRÍTICO) — GRAVEDAD separada de AVISO PREVIO.
//
// El arreglo de LEG-1 de esta misma ronda (commit 6ebfa53f) bloqueaba TODO
// evento de cámara de una unidad SIN viaje vivo antes de mirar si el evento
// era GRAVE (choque, volcadura): un choque o volcadura real de una unidad
// entre viajes ya no abría expediente de asistencia ni avisaba al jefe de
// tráfico — se perdía en silencio.
//
// Este archivo cubre los 4 cuadrantes: grave/rutinario × con/sin viaje vivo
// (y, para el caso grave, también con viaje vivo pero SIN aviso del operador
// — la urgencia física manda SIEMPRE, no solo cuando falta el viaje).
// ═══════════════════════════════════════════════════════════════════════════
      ...
      if (tabla === 'unidad') {
        ...
        api.then = (res: (v: unknown) => unknown) => res({
          data: estado.viajes.filter((v) =>
            ...
            (filtros['in:estatus'] as string[]).includes(v.estatus as string) &&
...
function conViajeVivo(aviso: boolean) {
  estado.viajes.push({ tenant_id: 't-1', unidad_id: 'u-1', operador_id: 'op-1', estatus: 'abierto' });
...
describe('LEG-OP-1 · un evento GRAVE no depende del aviso previo', () => {
  ...
  it('GRAVE + SIN viaje vivo: se guarda y dispara IGUAL — la urgencia física manda', async () => {
  ...
  it('GRAVE + CON viaje vivo pero SIN aviso del operador: se guarda y dispara IGUAL', async () => {
    conViajeVivo(false);
    const r = await sincronizarEventosDeFlota('t-1', 'samsara', CRED, samsaraCon([CHOQUE]), AHORA);
    expect(r.guardados).toBe(1);
    expect(r.disparos).toBe(1);
