...
insert into public.agente_definicion (id, nombre, departamento, disparador, estado, modelo_rol, prompt_ref, descripcion) values
  ...
  ('auditor_codigo',     'Auditor de código',            'ingenieria', 'cron',   'disenado', 'codigo', '10-Ingenieria-y-Producto/agente-auditor-de-codigo.md', 'AUDITA (hallazgos, no diffs). Motor VIVO desde el 16-ago: scripts/mejora-diaria/auditor.mjs (launchd 05:30, gpt-oss-120b) — caza y registra; el FIX lo escribe Claude Code por SUSCRIPCIÓN en el taller likida-mejoras y sale como PR a aprobación. Kill switch: .mejora-diaria/APAGADO.'),
  ...
  ('pruebas',            'Pruebas',                      'ingenieria', 'cron',   'disenado', 'codigo_escritura', '10-Ingenieria-y-Producto/agente-de-pruebas.md', 'Mantiene la suite — ESCRIBE código de prueba, así que corre con el rol codigo_escritura (frontera del 16-ago: modificar código escala a los mejores modelos). El diff va a aprobación.'),
...
-- ── Los 7 departamentos que faltaban (16-ago, "el sistema completo nivel YC") ──
-- Una startup autónoma no es solo vender y codificar: legal, seguridad,
-- fundraising, talento, alianzas, datos y la AUTOMEJORA que cierra el ciclo.
-- Blueprint escrito el mismo día para cada uno (prompt_ref) — estado disenado.
insert into public.agente_definicion (id, nombre, departamento, disparador, estado, modelo_rol, prompt_ref, descripcion) values
  ('legal_compliance',     'Legal y compliance',          'back_office', 'cron',   'disenado', 'analisis',    '09-Operaciones-Internas/agente-legal-compliance.md', 'LFPDPPP, ARCO, contratos y obligaciones corporativas de LIKIDA (no del producto — eso es de vigilancia normativa). Prepara; jamás firma ni contesta a una autoridad.'),
  ('talento',              'Talento',                     'back_office', 'manual', 'disenado', 'back_office', '09-Operaciones-Internas/agente-talento.md', 'Perfil, criba de CVs y onboarding interno cuando exista la primera vacante. Datos de candidatos = datos personales: RLS y ARCO.'),
  ('seguridad',            'Seguridad',                   'ingenieria',  'cron',   'disenado', 'codigo',      '10-Ingenieria-y-Producto/agente-de-seguridad.md', 'Deps, secretos y autorización multi-tenant (el patrón IDOR medido). HALLAZGOS con escenario de abuso; el FIX es de codigo_escritura.'),
  ('datos_instrumentacion','Datos e instrumentación',     'ingenieria',  'cron',   'disenado', 'analisis',    '10-Ingenieria-y-Producto/agente-datos-instrumentacion.md', 'Mapea qué pregunta de negocio no tiene dato y escribe la spec del evento. Generaliza el sinFuenteDeDatos del panel de adquisición.'),
  ('fundraising',          'Fundraising e inversionistas','direccion',   'cron',   'disenado', 'analisis',    '11-Alertas-y-Direccion/agente-fundraising.md', 'Investor update mensual con cifras del tablero, data room sin contradicciones, deadlines de aceleradoras. Todo sale por la cola de aprobación.'),
  ('automejora',           'Automejora',                  'direccion',   'cron',   'disenado', 'analisis',    '11-Alertas-y-Direccion/agente-automejora.md', 'El meta-agente: lee los diffs cuerpo/cuerpo_final (0117), los rechazos y las corridas fallidas, y propone el cambio de prompt A LA COLA. Umbral: 5 casos por patrón.'),
  ('alianzas',             'Alianzas y gremio',           'crecimiento', 'cron',   'disenado', 'back_office', '02-Ventas/agente-alianzas.md', 'CANACAR/ANPACT/TyT y aliados (contadores, vendedores SAP B1): calendario, mapa y acercamientos a la cola — el mismo circuito del redactor con otra lista.'),
  ('guiones',              'Guiones (scripts de video)',  'crecimiento', 'cron',   'disenado', 'marketing',   '06-Imagen-y-Video/agente-guiones.md', 'Etapa 1 de la cadena de video (MARCA.md §6): destila HOOKS de los videos de referencia de Javier (whisper local) al banco, y escribe el guion semanal a la cola. Motor: rutina local guiones-semanal (lunes 08:00). La cadena corre autónoma hasta las sequences; Javier autoriza sequence por sequence.'),
  ('noticias_mercado',     'Noticias del mercado',        'crecimiento', 'cron',   'disenado', 'marketing',   '07-Redes-Sociales/agente-noticias-mercado.md', 'Investiga a diario el mercado (flotas MX, competencia, tecnología) y lo convierte en carrusel con fuentes citadas a la cola de publicar (LinkedIn/IG/TikTok). Motor: rutina local noticias-diaria (09:00). Publicar es el tap de Javier.'),
  ('promos_diarias',       'Promos diarias',              'crecimiento', 'cron',   'disenado', 'marketing',   '07-Redes-Sociales/agente-promos-diarias.md', 'La pieza promocional del día: beneficios REALES de Likida en rotación, logo compuesto, copy por canal a la cola de publicar. Motor: rutina local promos-diaria (10:00). Sin clientes inventados, sin cifras sin fuente.'),
  ('experto_fiscal',       'Experto fiscal',              'producto',    'cron',   'disenado', 'analisis',    '10-Ingenieria-y-Producto/agente-experto-fiscal.md', 'DOF diario (SIDOF, 21:30: asienta en normas/ con fuente y escribe el fix de software si la norma lo exige; veredicto CAMBIO NORMATIVO) + profundidad dominical (re-verifica fichas contra fuente primaria, caza drift código-vs-ley). Motores: rutinas dof-diario y experto-fiscal.')
on conflict (id) do nothing;
